...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    if 80 <= efficiency <= 100:
        color = 'green'
    elif 60 <= efficiency <= 79:
        color = 'orange'
    elif 30 <= efficiency <= 59:
        color = 'red'
    elif 0 <= efficiency <= 30:
        color = 'black'
    else:
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    normal_low = threshold - threshold * 10/100
    normal_high = threshold + threshold * 10/100

    if coefficient < threshold * 40/100:
        status = 'LOW'
    elif normal_low <= coefficient <= normal_high:
        status = 'NORMAL'
    else:
        status = 'DANGER'
